...
public final class LogUtils {
    ...
    private static void log(final int type, final String tag, final Object... contents) {
        boolean condition = !sLogSwitch || (!sLog2ConsoleSwitch && !sLog2FileSwitch);
        if (condition) {
            return;
        }
        ...
        if (sLog2ConsoleSwitch && typeHigh != FILE) {
    ...
    private static void print2File(final int type, final String tag, final String msg) {
        final Format FORMAT = new SimpleDateFormat("MM-dd HH:mm:ss.SSS ", Locale.getDefault());
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && DxtSaleApplication.getInstance().getExternalCacheDir() != null) {
                sDefaultDir = DxtSaleApplication.getInstance().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            } else {
                sDefaultDir = DxtSaleApplication.getInstance().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
